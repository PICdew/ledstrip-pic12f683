...
/* static prototypes */
static void ir_rst(void);
static void ir_rst_all(void);
...
void ir_init(void) {
  ...
  TRISIO  = 0x04; // pin GP2/CCP1 as input / all others as output
  IRCF2   = 1;    // int osc: 8MHz
  IRCF1   = 1;    // "
  IRCF0   = 1;    // "
  PSA     = 1;    // No prescaler for timer0
  T0CS    = 0;    // Timer0 clock select: cpu clock
  T0IE    = 1;    // Timer0 irq enable
  TMR0    = 0;    // Clear timer0 register

  TMR1CS  = 0;    // Fosc/4
  TMR1IF  = 0;    // clear irq flag
  ...
  CCP1M3  = 0;    // Capture mode, every falling edge
  CCP1M2  = 1;    // "
  CCP1M1  = 0;    // "
  CCP1M0  = 0;    // "
  ir_flags.edge = 0;
...
/* reset irdata for various protocol (if needed) */
static void
ir_rst_all(void)
{
#if IR_SHARP_SUPPORT == 1
...
rc5_decode(void)
{
	if (ir_flags.first) {
		rc5.flags.raw = 0;
		rc5.irdata = 0x0005;	// we use rc5.irdata as a bit counter
		return;
	}
...
rc5_shift(void)
{
	...
	value = rc5.irdata & 0x1;
	if (!rc5.flags.same) value ^= 0x1;
	rc5.irdata = rc5.irdata << 1;
	rc5.irdata |= value;

	// check if we are done. rc5.irdata is used as bit counter
	if (rc5.irdata & 0x8000) {
		ir.type = IR_RC5;
		ir.command = (uint8_t)(rc5.irdata & 0x3f);
		ir.address = (rc5.irdata >> 6) & 0x1f;
		ir.extra = 0;
		if (rc5.irdata & 0x800) {
			ir.extra |= 0x80;
		} 
...
rc6_shift(void)
{
	...
	value = rc6.irdata & 0x1;
	if (!rc6.flags.same) value ^= 0x1;
	rc6.irdata = rc6.irdata << 1;
	rc6.irdata |= value;

	rc6.bitcount++;
	if (rc6.bitcount == 21) {
		ir.type = IR_RC6;
		ir.command = (uint8_t)(rc6.irdata & 0xff);
		ir.address = (uint16_t)((rc6.irdata >> 8) & 0xff);
		ir.extra = (uint8_t)((rc6.irdata >> 17) & 0x7);
		if (rc6.irdata & 0x10000) {
			ir.extra |= 0x80;
		}
		rc6.flags.last = 1;
...
jvc_decode(void)
{
	...
	if (jvc.flags.last) {
		if (ir_flags.timeout || CCPR1H > JVC_TIMEOUT) {
			// valid JVC !
			ir.type = IR_JVC;
			ir.address = jvc.irdata & 0xff;
			ir.command = (uint8_t)(jvc.irdata >> 8);
			ir.extra = 0;
			ir_flags.decoded = 1;
		} else {
			jvc.flags.invalid = 1;
...
nec_decode(void)
{
	...
	if (nec.flags.pulse) {
		...
		nec.flags.pulse = 0;
		if (nec.bitcount == 32) {
			ir.address = (uint16_t)(nec.irdata & 0xffff);
			...
			ir.command = (uint8_t)(nec.irdata >> 24);
			ir.extra = 0;
			nec.flags.last = 1;
		}
		return;
	...
	nec.irdata = nec.irdata >> 1;
	if (CCPR1H >= NEC_LONG_MIN) {
		nec.irdata |= 0x80000000;
	}
	nec.bitcount++;
}
...
sony_decode(void)
{
	...
	if (sony.flags.last) {
		if (ir_flags.timeout || CCPR1H > SONY_TIMEOUT) {
			// valid SONY !
			if (sony.bitcount == 12) {
				sony.irdata = sony.irdata >> 12;
				ir.type = IR_SONY_12BITS;
			} else if (sony.bitcount == 15) {
				sony.irdata = sony.irdata >> 9;
				ir.type = IR_SONY_15BITS;
			} else {
				sony.irdata = sony.irdata >> 4;
				ir.type = IR_SONY_20BITS;
			}
			ir.command = (uint8_t)(sony.irdata & 0x7f);
			ir.address = (uint16_t)(sony.irdata >> 7);
			ir.extra = 0;
			ir_flags.decoded = 1;
	...
	sony.irdata = sony.irdata >> 1;
	if (CCPR1H >= SONY_LONG_MIN) {
		sony.irdata |= 0x800000;
	}
	sony.bitcount++;
...
sharp_decode(void)
{
	if (ir_flags.first) {
		...
		sharp.flags.last = 0;
		sharp.flags.gap = 0;
		sharp.irdata = 0x8000;	// used as bit counter
		return;
	}
	...
	if (sharp.flags.last) {
		if (ir_flags.timeout) {
			...
			if (sharp.flags.second) {
				...
				temp0 = ~temp0 & 0x3;
				ir.extra = (uint8_t)(sharp.irdata >> 13);
				if (temp0 != ir.extra) {
					sharp.flags.invalid = 1;
					return;
				}
				...
				ir.command = (uint8_t)(sharp.irdata >> 5);
				if (temp0 != ~ir.command) {
					sharp.flags.invalid = 1;
					return;
				}
...
pace_decode(void)
{
	...
	if (pace.flags.last) {
		if (ir_flags.timeout) {
			 ...
			 */ 
			ir.address = (pace.irdata >> 5) & 0xf;
			ir.command = pace.irdata & 0x1f;
			ir.extra = (pace.irdata >> 9) & 0x1;
			ir_flags.decoded = 1;
		} else {
